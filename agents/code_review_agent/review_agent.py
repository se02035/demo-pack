...
# Nearest .env wins, walking up from this file. Process env is left unchanged.
load_dotenv()
...
async def review_code(target_dir: str, branch: str) -> dict:
    prompt = f"""Run `git diff ':!.github' ':!code_review_agent'` on branch {branch} in {target_dir} to get the changes on this branch.
...
if __name__ == "__main__":
    ...
    result = asyncio.run(review_code(target, branch))
